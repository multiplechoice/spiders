...
def test_postgres_statistics():
    ...
    pipeline = PostgresPipeline.from_crawler(crawler)
    job = JobsItem(
        spider="test", url="http://fake/job/advert.html", company="Project Blackwing"
    )
    spider = Spider("fake.com")

    # create a mock for the db session so we can control the response
    pipeline.session = mock.MagicMock()
    pipeline.session.query.return_value.filter.return_value.one_or_none.return_value = (
        None
    )
    ...
    pipeline.session.query.return_value.filter.return_value.one_or_none.return_value = ScrapedJob.from_dict(
        dict(job)
    )
    pipeline.process_item(job, spider)
...
def test_postgres_insert(db):
    crawler = get_crawler(settings_dict={"PG_CREDS": DIRK})
    pipeline = PostgresPipeline.from_crawler(crawler)
    test_subject = JobsItem(
        spider="a",
        url="http://subject.html",
        company="Project Blackwing",
        title="Test Subject",
    )
    director = JobsItem(
        spider="a",
        url="http://director.html",
        company="Project Blackwing",
        title="Director",
    )
    ...
    with session_scope(DIRK) as db_session:
        query = db_session.query(ScrapedJob).filter(
            ScrapedJob.url == "http://subject.html"
        )
        result = query.one()
        ...
        query = db_session.query(ScrapedJob).filter(
            ScrapedJob.url == "http://director.html"
        )
        result = query.one()
...
def test_image_downloader_elements():
    ...
    spider = Spider("fake.com")
    pipeline = ImageDownloader(
        "s3://frrp/", download_func=_mocked_download_func, settings=Settings(None)
    )
    pipeline.open_spider(spider)
    ...
    item = {
        ...
        "description": "<p> </p>",
        "image_urls": [
            "https://www.mbl.is/static/generic/2019/04/11/b4389bbc-0eca-4354-842b-80b2a05be489.jpg"
        ],
        "posted": "2019-04-11",
        "spider": "mbl",
